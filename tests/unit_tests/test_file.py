...
def test_package_installation():
    ...
    try:
        import src.house_pricing_predictor
    except ImportError as e:
        pytest.fail(f"Package import failed: {str(e)}")

    try:
        from src.house_pricing_predictor import main
    except ImportError as e:
        pytest.fail(f"Failed to import main module: {str(e)}")

    try:
        from src.house_pricing_predictor import data_ingestion
    except ImportError as e:
        pytest.fail(f"Failed to import DataIngestion module: {str(e)}")

    try:
        from src.house_pricing_predictor import model_training
    except ImportError as e:
        pytest.fail(f"Failed to import ModelTraining module: {str(e)}")

    try:
        from src.house_pricing_predictor import model_scoring
    except ImportError as e:
        pytest.fail(f"Failed to import ModelScoring module: {str(e)}")

    try:
        from src.house_pricing_predictor.data_ingestion import load_housing_data
